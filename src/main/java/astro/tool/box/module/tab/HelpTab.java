...
public class HelpTab {
    ...
    public void init() {
        try {
            ...
            text.newLine();
            text.row("There are two subtracted modes you can select from the 'Epochs' selection box. They work best if 'Set min/max limits' check box is checked. If you see nothing but black or");
            text.row("white images, try to pull the 'Min pixel value' slider carefully to the right or left until you see the difference image.");
            ...
            text.row("The slowest way is to load all the images (24) by selecting W1W2 and one of the following epochs: ALL, FIRST_REMAINING or YEAR.");
            text.row("But once all the images are loaded, you can switch between bands and epochs instantly.");
            text.newLine();
            text.row("Coverage maps may, for certain cases, improve the quality of bad coadds. When using them, all images have to be reloaded, which will take slightly more time because of their bigger size.");
            text.newLine();
            text.row("You may also skip low weighted coadds, which are then replaced by their counterpart of an earlier epoch, to improve the quality of the new coadds produced for one of the following epochs:");
            text.row("FIRST_LAST, FIRST_REMAINING or YEAR.");
            text.newLine();
