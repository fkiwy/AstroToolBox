...
public class Application {
	...
	public void init() {
		...
		PhotometricClassifierTab photoClassTab = new PhotometricClassifierTab(baseFrame, tabbedPane, catalogQueryTab,
				imageViewerTab);
		...
		ObjectCollectionTab objectCollectionTab = new ObjectCollectionTab(baseFrame, tabbedPane, catalogQueryTab,
				imageViewerTab);
		...
		if (!versionLoaded && checkVersion) {
			try {
				String response = readResponse(establishHttpConnection(RELEASES_URL + "versions.txt"),
						PGM_NAME + " version file");
				if (!response.isEmpty()) {
					...
					try (Scanner scanner = new Scanner(response)) {
						while (scanner.hasNextLine()) {
							...
							Version version = new Version(values[0], Boolean.parseBoolean(values[1]),
