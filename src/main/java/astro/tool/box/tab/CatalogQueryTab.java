			...
			if (!errorMessages.isEmpty()) {
				...
				CompletableFuture.supplyAsync(() -> {
					...
					try {
						...
						String searchLabelText = "RA=" + targetRa + "° dec=" + targetDec + "° radius=" + searchRadius
								+ " arcsec";
						if (count > 0) {
							searchLabel.setText(
									count + " result(s) for " + searchLabelText + " (" + resultsPerCatalog + ")");
						} else {
	...
	private void displayCatalogResults(List<CatalogEntry> catalogEntries) {
		...
		catalogTable.getSelectionModel().addListSelectionListener((ListSelectionEvent e) -> {
			if (!e.getValueIsAdjusting()) {
				...
				CatalogEntry selected = catalogEntries.stream().filter(entry -> entry.getSourceId().equals(sourceId))
						.findFirst().get();
				if (selected != null) {
		...
		container.setBorder(BorderFactory.createTitledBorder(new LineBorder(catalogEntry.getCatalogColor(), 3),
	...
	private void displaySpectralTypes(CatalogEntry catalogEntry, boolean addExtinctionCheckbox) {
		try {
			...
			collectButton.addActionListener((ActionEvent evt) -> {
				...
				collectObject(selectedObjectType, catalogEntry, baseFrame, mainSequenceSpectralTypeLookupService,
						collectionTable);
					...
					if (dustExtinction.isSelected()) {
						...
						try {
							Map<String, Double> extinctionsByBand = dustExtinctionService
									.getExtinctionsByBand(entry.getRa(), entry.getDec(), 2.0);
							try {
								...
								extinctionPanel.add(createLabel(
										"No extinction values for " + entry.getCatalogName() + " bands.", JColor.RED));
							}
						} catch (Exception ex) {
							showExceptionDialog(baseFrame, ex);
						} finally {
