...
public class BatchQueryTab implements Tab {
		...
		try {
			...
			queryButton.addActionListener((ActionEvent evt) -> {
				...
				if (rows > MAX_INPUT_ROWS) {
					showErrorDialog(baseFrame,
							"The selected file contains " + rows + " rows! Max. allowed: " + MAX_INPUT_ROWS);
					return;
				}
				...
				try {
					...
					if (searchRadius > MAX_SEARCH_RADIUS) {
						errors.append("Radius must not be larger than ").append(MAX_SEARCH_RADIUS).append(" arcsec.")
								.append(LINE_SEP);
					}
				} catch (Exception ex) {
			...
			exportButton.addActionListener((ActionEvent evt) -> {
				if (batchResults == null || batchResults.isEmpty()) {
					...
					String content = batchResults.stream().map(result -> result.getValues())
		...
		try (Scanner scanner = new Scanner(file)) {
			...
			while (scanner.hasNextLine()) {
				...
				while (iter.hasNext()) {
					...
					List<String> spectralTypes = lookupSpectralTypes(catalogEntry.getColors(true),
							spectralTypeLookupService, includeColors.isSelected());
					...
					BatchResult batchResult = new BatchResult.Builder().setRowNumber(rowNumber)
							.setObjectNumber(objectNumber).setCatalogName(catalogEntry.getCatalogName())
							.setTargetRa(targetRa).setTargetDec(targetDec)
							...
							.setPlx(catalogEntry.getPlx()).setPmra(catalogEntry.getPmra())
							.setPmdec(catalogEntry.getPmdec()).setMagnitudes(catalogEntry.getMagnitudes())
	...
	private void displayQueryResults() {
		...
		resultScrollPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),
				"Batch query results", TitledBorder.LEFT, TitledBorder.TOP));
