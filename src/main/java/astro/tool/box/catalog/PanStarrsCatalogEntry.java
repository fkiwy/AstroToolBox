...
public class PanStarrsCatalogEntry implements CatalogEntry {
	...
	public void loadCatalogElements() {
		catalogElements.add(new CatalogElement("dist (arcsec)", roundTo3DecNZLZ(getTargetDistance()), Alignment.RIGHT,
				getDoubleComparator()));
		catalogElements
				.add(new CatalogElement("source id", String.valueOf(objID), Alignment.LEFT, getLongComparator()));
		catalogElements.add(new CatalogElement("object name", objName, Alignment.LEFT, getStringComparator()));
		catalogElements.add(new CatalogElement("quality flag sum", String.valueOf(qualityFlag), Alignment.RIGHT,
				getIntegerComparator(), createToolTipQualityFlag()));
		catalogElements.add(new CatalogElement("ra", roundTo7DecNZ(raMean), Alignment.LEFT, getDoubleComparator()));
		catalogElements.add(
				new CatalogElement("ra err (arcsec)", roundTo4DecNZ(raMeanErr), Alignment.LEFT, getDoubleComparator()));
		catalogElements.add(new CatalogElement("dec", roundTo7DecNZ(decMean), Alignment.LEFT, getDoubleComparator()));
		catalogElements.add(new CatalogElement("dec err (arcsec)", roundTo4DecNZ(decMeanErr), Alignment.LEFT,
				getDoubleComparator()));
		catalogElements.add(new CatalogElement("mean observ. time", epochMean.format(DATE_TIME_FORMATTER),
				Alignment.LEFT, getStringComparator()));
		catalogElements.add(
				new CatalogElement("detections", String.valueOf(nDetections), Alignment.RIGHT, getIntegerComparator()));
		catalogElements
				.add(new CatalogElement("g (mag)", roundTo3DecNZ(gMeanPSFMag), Alignment.RIGHT, getDoubleComparator()));
		catalogElements.add(
				new CatalogElement("g err", roundTo3DecNZ(gMeanPSFMagErr), Alignment.RIGHT, getDoubleComparator()));
		catalogElements
				.add(new CatalogElement("r (mag)", roundTo3DecNZ(rMeanPSFMag), Alignment.RIGHT, getDoubleComparator()));
		catalogElements.add(
				new CatalogElement("r err", roundTo3DecNZ(rMeanPSFMagErr), Alignment.RIGHT, getDoubleComparator()));
		catalogElements
				.add(new CatalogElement("i (mag)", roundTo3DecNZ(iMeanPSFMag), Alignment.RIGHT, getDoubleComparator()));
		catalogElements.add(
				new CatalogElement("i err", roundTo3DecNZ(iMeanPSFMagErr), Alignment.RIGHT, getDoubleComparator()));
		catalogElements
				.add(new CatalogElement("z (mag)", roundTo3DecNZ(zMeanPSFMag), Alignment.RIGHT, getDoubleComparator()));
		catalogElements.add(
				new CatalogElement("z err", roundTo3DecNZ(zMeanPSFMagErr), Alignment.RIGHT, getDoubleComparator()));
		catalogElements
				.add(new CatalogElement("y (mag)", roundTo3DecNZ(yMeanPSFMag), Alignment.RIGHT, getDoubleComparator()));
		catalogElements.add(
				new CatalogElement("y err", roundTo3DecNZ(yMeanPSFMagErr), Alignment.RIGHT, getDoubleComparator()));
	...
	public String[] getColumnValues() {
		String columnValues = roundTo3DecLZ(getTargetDistance()) + "," + objID + "," + objName + "," + qualityFlag + ","
				+ roundTo7Dec(raMean) + "," + roundTo4Dec(raMeanErr) + "," + roundTo7Dec(decMean) + ","
				+ roundTo4Dec(decMeanErr) + "," + epochMean.format(DATE_TIME_FORMATTER) + "," + nDetections + ","
				+ roundTo3Dec(gMeanPSFMag) + "," + roundTo3Dec(gMeanPSFMagErr) + "," + roundTo3Dec(rMeanPSFMag) + ","
				+ roundTo3Dec(rMeanPSFMagErr) + "," + roundTo3Dec(iMeanPSFMag) + "," + roundTo3Dec(iMeanPSFMagErr) + ","
				+ roundTo3Dec(zMeanPSFMag) + "," + roundTo3Dec(zMeanPSFMagErr) + "," + roundTo3Dec(yMeanPSFMag) + ","
				+ roundTo3Dec(yMeanPSFMagErr) + "," + roundTo3Dec(get_g_r()) + "," + roundTo3Dec(get_r_i()) + ","
				+ roundTo3Dec(get_i_z()) + "," + roundTo3Dec(get_z_y());
	...
	public double getTargetDistance() {
		return calculateAngularDistance(new NumberPair(targetRa, targetDec), new NumberPair(raMean, decMean),
				DEG_ARCSEC);
	}
	...
	public double get_g_err() {
		return gMeanPSFMagErr;
	}

	public double get_r_err() {
		return rMeanPSFMagErr;
	}

	public double get_i_err() {
		return iMeanPSFMagErr;
	}

	public double get_z_err() {
		return zMeanPSFMagErr;
	}

	public double get_y_err() {
		return yMeanPSFMagErr;
	}
