...
public class AstrometricFunctions {
	...
	public static double calculateTangentialVelocityFromParallax(double pmRA, double pmDE, double parallax) {
		return VELOCITY_C * (calculateTotalProperMotion(pmRA, pmDE) / ARCSEC_MAS)
				* calculateParallacticDistance(parallax);
	}
	...
	public static NumberPair convertToGalacticCoords(double raEquatorial, double decEquatorial) {
		...
		double glon = glon0
				- atan((cos(dec) * sin(ra - ra0)) / (sin(dec) * cos(dec0) - cos(dec) * sin(dec0) * cos(ra - ra0)));
	...
	public static LocalDateTime convertMJDToDateTime(BigDecimal mjd) {
		...
		BigInteger nanos = decimalSeconds.subtract(new BigDecimal(integerSeconds))
				.multiply(new BigDecimal(1_000_000_000L)).toBigInteger();
		Duration duration = Duration.ofDays(days.longValue()).plusSeconds(integerSeconds.longValue())
				.plusNanos(nanos.longValue());
	...
	public static double calculateTotalProperMotionError(double a, double ae, double b, double be) {
		return sqrt(pow(((a * 2) / (2 * sqrt(a * a + b * b))) * ae, 2)
