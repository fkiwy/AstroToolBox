			...
			searchButton.addActionListener((ActionEvent e) -> {
				try {
					...
					if (targetRa == prevTargetRa && targetDec == prevTargetDec && searchRadius == prevSearchRadius
							&& numberOfRows == prevNumberOfRows && allColumns == prevAllColumns) {
						return;
					}
					position = 0;
					matchesFound = 0;
					...
					if (!errorMessages.isEmpty()) {
						...
						CompletableFuture.supplyAsync(() -> {
							try {
								...
								url = url.formatted(Double.toString(targetRa), addPlusSign(targetDec), searchRadius,
										numberOfRows, outAll);
								...
								JLabel vizierLink = createHyperlink("Open in web browser", getVizierUrl(targetRa,
										targetDec, searchRadius, numberOfRows, allColumnsCheckBox.isSelected()));
