...
public class BatchResult {
	...
	public String getValues() {
		String values = rowNumber + "," + suppressZero(objectNumber) + "," + catalogName + "," + roundTo7Dec(targetRa)
				+ "," + roundTo7Dec(targetDec) + ","
				+ (catalogName.isEmpty() ? roundTo3Dec(targetDistance) : roundTo3DecLZ(targetDistance)) + ","
				+ roundTo7Dec(ra) + "," + roundTo7Dec(dec) + "," + sourceId + "," + roundTo4Dec(plx) + ","
				+ roundTo3Dec(pmra) + "," + roundTo3Dec(pmdec) + "," + magnitudes + "," + joinSpetralTypes();
		return values;
	}
	...
	public static class Builder {
		...
		public BatchResult build() {
			return new BatchResult(rowNumber, objectNumber, catalogName, targetRa, targetDec, targetDistance, ra, dec,
					sourceId, plx, pmra, pmdec, magnitudes, spectralTypes);
		}
